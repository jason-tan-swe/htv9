...
const SocketPage = () => {
  ...
  useEffect(() => {
    const onPactJoin = (args) => {
      console.log(args);
      const { players, pactId, hasPlayerOneConfirmed, hasPlayerTwoConfirmed } =
        args;
      setPlayers(players);
  ...
  return (
    <div className="flex flex-col p-2 gap-4">
      <input
        className="border border-green-300 b-2 "
        type="text"
        value={pactId}
        onChange={(e) => setPactId(e.target.value)}
      />
      <button className="bg-green-300 px-4 py-2" onClick={handleJoinRoom}>
        Join Room
      </button>
      <button className="bg-green-300 px-4 py-2" onClick={handleCreateRoom}>
        Create Room
      </button>
      <button className="bg-green-300 px-4 py-2" onClick={handleReadyRoom}>
        Ready
      </button>
      {players.map((player, index) => (
        <p key={player._id}>
          {player.email}{" "}
          {index === 0
            ? `Hello ${hasPlayerOneConfirmed}`
            : `Hello 2${hasPlayerTwoConfirmed}`}{" "}
        </p>
      ))}
    </div>
  );
